...
try {
    $sql = "SELECT 
        ...
        p.status,
        p.created_at,
        DATEDIFF(NOW(), p.created_at) as days_overdue,
        CASE 
            WHEN DATEDIFF(NOW(), p.created_at) <= 30 THEN 'CURRENT'
            WHEN DATEDIFF(NOW(), p.created_at) <= 60 THEN '31-60'
            WHEN DATEDIFF(NOW(), p.created_at) <= 90 THEN '61-90'
            ELSE '90+'
        END as age_bucket
    FROM payables p
    WHERE p.status IN ('PENDIENTE', 'PAGADO')
